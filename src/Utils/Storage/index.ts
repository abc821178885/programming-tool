...
let LocalStorage: FLStorage<unknown> | null = null;
let SessionStorage: FLStorage<unknown> | null = null;

/**
 * 获取本地存储 (`localStorage`) 的 `FLStorage` 实例。
 * 如果实例尚未创建且处于客户端环境（即 `window` 对象存在），则会创建并返回该实例。
 * @return {FLStorage<unknown> | null} 返回 `FLStorage` 实例或者在非客户端环境下返回 `null`。
 */
const getLocalStorage = () => {
  if (!LocalStorage && typeof window !== "undefined") {
    LocalStorage = new FLStorage(window.localStorage);
  }
  return LocalStorage;
};

/**
 * 获取会话存储 (`sessionStorage`) 的 `FLStorage` 实例。
 * 如果实例尚未创建且处于客户端环境（即 `window` 对象存在），则会创建并返回该实例。
 * @return {FLStorage<unknown> | null} 返回 `FLStorage` 实例或者在非客户端环境下返回 `null`。
 */
const getSessionStorage = () => {
  if (!SessionStorage && typeof window !== "undefined") {
    SessionStorage = new FLStorage(window.sessionStorage);
  }
